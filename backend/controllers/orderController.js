...
const addOrderItems = asyncHandler(async (req, res) => {
  ...
  if (orderItems && orderItems.length === 0) {
    ...
    // Kiểm tra số lượng và giảm số lượng sản phẩm
    const updates = itemsFromDB.map(async dbItem => {
      const orderItem = orderItems.find(item => item._id === dbItem._id.toString());
      if (dbItem.countInStock < orderItem.qty) {
        throw new Error(`Not enough stock for ${dbItem.name}`);
      }
      dbItem.countInStock -= orderItem.qty;
      await dbItem.save();
    });
